...
public class MoveBehaviour : GenericBehaviour
{
    ...
    private float pvH = 0;
    private float pvV = 0;
    public bool bounceFromCollide = false;
    ...
    void Start()
    {
        ...
        behaviourManager.setH = Random.Range(-1f, 1f);
        behaviourManager.setV = Random.Range(-1f, 1f);
    ...
    public override void LocalFixedUpdate()
    {
        ...
        {
            agent.SetDestination(new Vector3(-61.0f,0,-6.6f));
            
            if(agent.remainingDistance > agent.stoppingDistance)
            {

                /*
                behaviourManager.GetAnim.SetFloat(Animator.StringToHash("H"), agent.velocity.x);
                behaviourManager.GetAnim.SetFloat(Animator.StringToHash("V"), agent.velocity.z);
                behaviourManager.GetAnim.SetFloat(Animator.StringToHash("Speed"), runSpeed);*/
                //MovementManagement(agent.nextPosition.x, agent.nextPosition.z);
               // behaviourManager.GetAnim.Set
            }
            ...
            if (IsInValidArea())
            {
                ...
                if (timeToChangeDirection <= 0)
                {
                    ...
                    pvH = behaviourManager.GetH;
                    pvV = behaviourManager.GetV;
                    //jump = true;
                    timeToChangeDirection = 10f;

                }else if (isColliding)
                {
                    behaviourManager.setH = -pvH;
                    behaviourManager.setV = -pvV;

                    pvH = behaviourManager.GetH;
                    pvV = behaviourManager.GetV;

                    bounceFromCollide = true;
                    positionOfCollision = transform.position;
                    timeToChangeDirection = 10f;
                }
            }
            else
            {
                ...
                float offsetX = 3 * ox < 0 ? -1 : 1;
                float offsetZ = 3 * oz < 0 ? -1 : 1;

                Vector3 newPosition = new Vector3(transform.position.x - offsetX, transform.position.y, transform.position.z - offsetZ);
    ...
    {
        rotateDirection = getRandomH();
        isColliding = false;
    }


    private float getRandomH()
    {
        float nH = 1;
